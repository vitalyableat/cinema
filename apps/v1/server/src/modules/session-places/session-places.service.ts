...
export class SessionPlacesService {
  ...
  async create(createSessionPlaceDto: CreateSessionPlaceDto): Promise<SessionPlace> {
    const session = await this.sessionsRepository.findOneBy({
      id: createSessionPlaceDto.sessionId,
    });
    const lounge = await this.loungesRepository.findOneBy({
      id: createSessionPlaceDto.loungeId,
    });
    const seat = await this.seatsRepository.findOneBy({
      id: createSessionPlaceDto.seatId,
    });
  ...
  async update(id: number, updateSessionPlaceDto: UpdateSessionPlaceDto): Promise<SessionPlace> {
    ...
    const session = await this.sessionsRepository.findOneBy({
      id: updateSessionPlaceDto.sessionId,
    });
    const lounge = await this.loungesRepository.findOneBy({
      id: updateSessionPlaceDto.loungeId,
    });
    const seat = await this.seatsRepository.findOneBy({
      id: updateSessionPlaceDto.seatId,
    });

    return await this.entityManager.save(
      new SessionPlace({
        ...sessionPlace,
        ...updateSessionPlaceDto,
        session,
        lounge,
        seat,
      }),
