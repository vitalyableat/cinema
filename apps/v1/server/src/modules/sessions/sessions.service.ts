...
export class SessionsService {
  ...
  async create(createSessionDto: CreateSessionDto): Promise<Session> {
    ...
    const language = await this.languagesRepository.findOneBy({
      id: createSessionDto.languageId,
    });
    const show = await this.showsRepository.findOneBy({
      id: createSessionDto.showId,
    });
    const lounge = await this.loungesRepository.findOneBy({
      id: createSessionDto.loungeId,
    });

    const loungePlaces = await this.placesRepository.find({
      where: { lounge },
      relations: { seat: true },
    });
  ...
  async update(id: number, updateSessionDto: UpdateSessionDto): Promise<Session> {
    ...
    const language = await this.languagesRepository.findOneBy({
      id: updateSessionDto.languageId,
    });
    const show = await this.showsRepository.findOneBy({
      id: updateSessionDto.showId,
    });
    const lounge = await this.loungesRepository.findOneBy({
      id: updateSessionDto.loungeId,
    });
